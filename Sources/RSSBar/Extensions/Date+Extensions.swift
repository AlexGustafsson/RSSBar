...
extension Date {
  func formattedDistance(to other: Date) -> String {
    ...
    if seconds < 60 { return "\(prefix)\(Int(seconds))s\(postfix)" }

    let minutes = floor(seconds / 60)
    if minutes < 60 { return "\(prefix)\(Int(minutes))m\(postfix)" }

    let hours = floor(minutes / 60)
    if hours < 60 { return "\(prefix)\(Int(hours))h\(postfix)" }

    let days = floor(hours / 24)
    if days < 365 { return "\(prefix)\(Int(days))d\(postfix)" }
